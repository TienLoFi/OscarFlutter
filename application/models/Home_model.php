...
Class Home_model extends BaseModel {
	...
	public function setReset($id) {
		...
		$query = "UPDATE tb_users SET score=0, last_update=".time()." WHERE id=$id";
		$query = $this->db->query($query);
	}
	...
	public function getAnswer($user_id) {
		...
		$query = "SELECT * FROM tb_users WHERE level=100 LIMIT 1";
		$query = $this->db->query($query);
	...
	public function isExist($id='') {
		...
		$query = "SELECT id FROM tb_ballots WHERE id='$id'";
		$query = $this->db->query($query);
