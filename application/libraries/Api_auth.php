...
class Api_Auth
{
    private $encryptKey = 'gdgdsnfg54n5SSSt594g54gn4g04gj09j5gh043gh3403hj'; //random generated secret key
    private $secretIV = '5g8g5jg59jjhSSSSgh569538545t90yjyh04yj4y606ygfmhkgf'; //random generated initialization vector

    
    private $isTokenExpire=true; //set true for token expiry, false for never expire
    private $tokenExpriryInHours=24*365; //token expiry time in Hours
    ...
    */
    ...
    function storeTokenInAuthTokens($userId,$token)
    {
        ...
        $CI= &get_instance();  //Store instance in a variable.
        $CI->load->database();
        if($CI->db->query("SELECT * FROM tb_auth_tokens WHERE user_id='$userId'")->num_rows() > 0)
        {
            $CI->db->where('user_id',$userId);
            $CI->db->update('tb_auth_tokens',['expiry_date' =>$expirtyDate,'user_id'=>$userId,'token'=>$token,'created_at'=>date('Y-m-d H:i')]);
        }
        else 
        {
            $CI->db->insert('tb_auth_tokens',['expiry_date' =>$expirtyDate,'user_id'=>$userId,'token'=>$token,'created_at'=>date('Y-m-d H:i')]);
        }
    }

    function findAuthToken($token, $userId)
    {
        $CI= &get_instance();  //Store instance in a variable.
        $CI->load->database();
        $query = $CI->db->query("SELECT * FROM tb_auth_tokens WHERE token='$token' AND user_id=$userId");
        $result = $query->result();
